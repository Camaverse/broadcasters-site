    ...
    header.header-global(:class="{hasDrop, hasBackground}")
        ...
        b-navbar.nav-primary.hasTagList(toggleable='lg', type='dark')
            ...
            b-collapse#nav_collapse(is-nav='')
                b-navbar-nav
                    b-nav-item(v-if='searchRoutes' v-for='rt in searchRoutes', :key='rt', :to='"/" + rt.toLowerCase()') {{ $t(rt) }}
                    b-nav-item(:to='"/faves"', v-if="isLoggedIn") Faves
                    b-nav-item(:to='"/recent"', v-if="showRecent") Recent
                b-navbar-nav.ml-auto
                    b-nav-form
                        b-form-input.mr-sm-2(size='sm', type='text', :placeholder='$t("Search")')
                        b-button.my-2.my-sm-0(size='sm', type='submit') {{ $t('Search') }}
                b-navbar-nav.ml-auto
                    locale-changer
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      showRecent: state => state.recent.rooms.list.length,
